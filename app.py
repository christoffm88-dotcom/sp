...
# Automatisch het bestand inlezen met automatische scheidingsteken-detectie
bestand_naam = "gereedschap.csv"

if os.path.exists(bestand_naam):
  try:
    # sep=None en engine='python' zorgen ervoor dat komma's én puntkomma's automatisch werken
    df = pd.read_csv(bestand_naam, sep=None, engine="python")
  except Exception as e:
    df = pd.DataFrame(
        columns=["Naam", "Categorie", "Ligging", "Datum toegevoegd", "Foto"]
    )
    st.error(
        "⚠️ Kon de CSV niet automatisch lezen. Controleer of het bestand"
        " correct is opgemaakt."
    )

  # Zorg dat de basiskolommen altijd bestaan
